...
for (i in 1:nsim) {
  for (j in 1:length(xind)) {
    ...
    sd_2 <- 1/(sum(S1$censor[S1$trt==0])) + 1/(sum(S1$censor[S1$trt==1]))
    sd_3 <- 1/(sum(S2$censor[S2$trt==0])) + 1/(sum(S2$censor[S2$trt==1]))
    sd_4 <- 1/(sum(S3$censor[S3$trt==0])) + 1/(sum(S3$censor[S3$trt==1]))
    ...
    results_list[[counter]] <- list(
      ...
      posterior_sd_theta = posterior_sd_theta,
      posterior_lb_theta = posterior_lb_theta,
      posterior_ub_theta = posterior_ub_theta,
