...
/* KM plot - time to death*/
%macro kmos(in,out);
data adtte;
  set &in;
  if dthd90 = 1 then cnsr= 0;
  else cnsr= 1;
  aval=t2dthd90;
  grpn=rangp;
...
proc sql noprint;
   select max(AVAL) into: maxsur1 separated by "" from ADTTE;
quit;	
...
proc lifetest data=ADTTE method=km plots=survival(atrisk=0 to &maxsur. by 5)
...
proc template;
  ...
  yaxisopts=(griddisplay=off label="Probability of OS (%)" linearopts=(tickvaluepriority=true 
  ...
  layout gridded/columns=3 border=false autoalign=auto columngutter=1;
    ...
    entry halign=left "Median OS,(95% CI)"/TEXTATTRS=(size=8pt Family= 'Courier New'); entry halign=left "&medci1."/TEXTATTRS=(size=8pt);  entry halign=left "&medci2."/TEXTATTRS=(size=8pt); 
  endlayout;
...
%kmos(in=az,out=az_km_os);
%kmos(in=brii,out=brii_km_os);
%kmos(in=lilly,out=lilly_km_os);
%kmos(in=mp,out=mp_km_os);
%kmos(in=pf,out=pf_km_os);
%kmos(in=vir,out=vir_km_os);
