...
data adtte2;
  set &in;
  if recovery in (0,2) then cnsr= 1;
  else if recovery = 1 then cnsr= 0;
  aval=t2recovery;
  grpn=rangp;
run;
...
proc template;
  ...
  yaxisopts=(griddisplay=off label="Probability of non-recovery (%)" linearopts=(tickvaluepriority=true 
  ...
  layout gridded/columns=3 border=false autoalign=auto columngutter=1;
    ...
    entry halign=left "Median non-recovery,(95% CI)"/TEXTATTRS=(size=8pt Family= 'Courier New'); entry halign=left "&medci1."/TEXTATTRS=(size=8pt);  entry halign=left "&medci2."/TEXTATTRS=(size=8pt); 
  endlayout;
...
%kmrecov(in=az,out=az_km_recov);
%kmrecov(in=brii,out=brii_km_recov);
%kmrecov(in=lilly,out=lilly_km_recov);
%kmrecov(in=mp,out=mp_km_recov);
%kmrecov(in=pf,out=pf_km_recov);
%kmrecov(in=vir,out=vir_km_recov);
