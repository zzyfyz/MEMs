...
dat1 <- subset(dat, cohort==1|cohort==2)
dat2 <- subset(dat, cohort==1|cohort==3)
dat2$cohort <- ifelse(dat2$cohort!=1,2,1)
dat3 <- subset(dat, cohort==1|cohort==4)
dat3$cohort <- ifelse(dat3$cohort!=1,2,1)
dat4 <- subset(dat, cohort==1|cohort==5)
dat4$cohort <- ifelse(dat4$cohort!=1,2,1)
dat5 <- subset(dat, cohort==1|cohort==6)
dat5$cohort <- ifelse(dat5$cohort!=1,2,1)
...
prior_post <- function(input,S,mod.mat,K,tau_p,cuttype){
  ### Derive values from data
  data <- input
  S <- S                              
  T0 <- S                             # max number of distinct cohorts to consider
  N <- nrow(data)                      # total number of subjects
  ...
  if (cuttype == "primary") {
    ...
    data$interval <- apply( cbind(data$time), 1, function(x){ sum(x > int.cuts) } )
  }
  else if (cuttype == "equal") {
    
    int.cuts <- c( 0, max(data$time)/K, max(data$time)/K*2, max(data$time)/4*3)
    data$interval <- apply( cbind(data$time), 1, function(x){ sum(x > int.cuts) } )
  }
  else if (cuttype == "events") {
    
    int.cuts <- c( 0, quantile( data$time[which(data$censor == 1)],
  ...
  for(i in 1:S){
    W.trt.mat[,i] <- ifelse( data$trt == 1 & data$cohort == unique(data$cohort)[i], 1, 0 )
  }
  ...
  alpha0 <- 3                                     # pre-specified value for prior model probabilities
  
  
  #Model prior from bmabasket approach
  construct_modPriors <- function(S, T0, mod_mat, alpha = alpha0){
  ...
  for(i in 1:S){
    ...
    dat.i <- data[ which(data$cohort == i), ]    # data for only cohort i
  ...
  for(i in 1:S){
    for(k in 1:K){
      d.ijk.times.nu.ij <- ifelse( data$cohort == i & data$censor == 1 &
                                     data$interval == k, 1, 0 )
  ...
  n.i <- table(data$cohort)              # cohortal sample size
  ...
  #ss
  my_list<- list("pmp"=pmp.vec,"mean"=rte.means,"sd"=rte.sds,"ss"=ss)
  return(my_list)
}

##get empirical prior

p_1 <- prior_post(dat1,2,rbind(c(1,1),c(1,2)),2,c(0, 24, 90),cuttype="primary")
p_2 <- prior_post(dat2,2,rbind(c(1,1),c(1,2)),2,c(0, 24, 90),cuttype="primary")
p_3 <- prior_post(dat3,2,rbind(c(1,1),c(1,2)),2,c(0, 24, 90),cuttype="primary")
p_4 <- prior_post(dat4,2,rbind(c(1,1),c(1,2)),2,c(0, 24, 90),cuttype="primary")
p_5 <- prior_post(dat5,2,rbind(c(1,1),c(1,2)),2,c(0, 24, 90),cuttype="primary")


#main functions
#S: number of total cohorts(primary + supplemental)
...
main <- function(S,mod.mat,K,tau_p,cuttype,priortype,W,alpha0){
  ...
  if (cuttype == "primary") {
    ...
    interval_breaks <- seq(0, max(dat$time), length.out = K+1)
    int.cuts <- interval_breaks[1:K]
    dat$interval <- apply( cbind(dat$time), 1, function(x){ sum(x > int.cuts) } )
  ...
  if (priortype == 1) {
    ...
    W <- cbind(p_1$pmp[1],p_2$pmp[1],p_3$pmp[1],p_4$pmp[1],p_5$pmp[1])
...
mod <- main(6,rbind(
  ...
  c(1, 1, 2, 3, 4, 5),
  c(1, 2, 3, 4, 5, 6)
),2,c(0, 24, 90),cuttype="primary",priortype=3,W=NA,alpha0=NA)
...
write.csv(effm, "./effm_7.csv", row.names=FALSE)
write.csv(pmp, "./pmp_7.csv", row.names=FALSE)
write.csv(ss, "./ss_7.csv", row.names=FALSE)
write.csv(effci, "./ci_7.csv", row.names=FALSE)
